...
def _handle_pipeline_commands(makim_instance: Makim) -> typer.Typer:
    ...
    def run(
        ...
        parallel: bool = typer.Option(
            False, '--parallel', help='Run pipeline in parallel'
        ),
        sequential: bool = typer.Option(
            False, '--sequential', help='Run pipeline sequentially'
        ),
        dry_run: bool = typer.Option(
            False, '--dry-run', help='Simulate execution without running tasks'
        ),
        debug: bool = typer.Option(
            False, '--debug', help='Enable detailed debug output'
        ),
        max_workers: Optional[int] = typer.Option(
            None, '--max-workers', help='Limit parallel execution workers'
        ),
    ) -> None:
        """Execute a single pipeline in sequential or parallel mode."""
        if parallel and sequential:
            typer.echo(
                '❌ Conflicting options: --parallel and --sequential cannot be used together.'
            )
            raise typer.Exit(1)
        ...
        if debug:
            typer.echo(
                f"🔍 DEBUG MODE ENABLED: Running pipeline '{name}' with detailed logs"
            )

        if parallel:
            typer.echo(f"🚀 Running pipeline '{name}' in PARALLEL mode...")
            engine.run_pipeline_parallel(
                name,
                steps,
                debug=debug,
            ...
            engine.run_pipeline_sequential(
                name, steps, debug=debug, dry_run=dry_run
            )
    ...
    def run_all(
        parallel: bool = typer.Option(
            False, '--parallel', help='Run all pipelines in parallel'
        ),
        sequential: bool = typer.Option(
            False, '--sequential', help='Run all pipelines sequentially'
        ),
        max_workers: Optional[int] = typer.Option(
            None, '--max-workers', help='Limit parallel execution workers'
        ),
    ...
    def _execute_pipeline(
        name, pipeline_data, parallel, sequential, dry_run, debug, max_workers
    ):
    ...
    def logs(
        pipeline: Optional[str] = typer.Argument(
            None, help='Pipeline name to filter logs'
        ),
        named_pipeline: Optional[str] = typer.Option(
            None, '--pipeline', '-p', help='Pipeline name to filter logs'
        ),
        last: int = typer.Option(
            10, '--last', '-l', help='Number of recent logs to display'
        ),
        clear: bool = typer.Option(
            False, '--clear', '-c', help='Clear all pipeline logs'
        ),
        ...
        status: Optional[str] = typer.Option(
            ...
            '--status',
            '-s',
            help="Filter logs by status (e.g., 'failed', 'success')",
        ),
    ) -> None:
        ...
        if follow:
            typer.echo(
                f"📡 Following logs for pipeline '{pipeline_name or 'ALL'}'... (Press Ctrl+C to stop)"
            )
    ...
    @typer_pipeline.command(
        help='Schedule a pipeline for automatic execution.'
    )
    def schedule(
        name: str = typer.Argument(..., help='Pipeline name to schedule'),
        cron: Optional[str] = typer.Option(
            None, help="Cron expression (e.g., '0 9 * * *' for daily at 9 AM)"
        ),
        interval: Optional[int] = typer.Option(
            None, help='Interval in seconds'
        ),
    ) -> None:
        """Schedule a pipeline execution using cron or interval."""
        if not cron and not interval:
            typer.echo(
                '❌ Please provide either a cron expression (--cron) or an interval (--interval).'
            )
            raise typer.Exit(1)

        engine = MakimPipelineEngine(config_file=makim_instance.file)
        engine.schedule_pipeline(
            name, cron_expression=cron, interval_seconds=interval
        )
    ...
    def history(
        name: Optional[str] = typer.Argument(
            None, help='Pipeline name to filter history.'
        ),
        limit: int = typer.Option(
            10, help='Number of past executions to show.'
        ),
    ) -> None:
        ...
        if not history_data:
            typer.echo(
                f"❌ No execution history found for pipeline '{name}'."
                if name
                else 'No execution history found.'
            )
            return
    ...
    def retry(
        ...
        all_failed: bool = typer.Option(
            False, '--all', help='Retry all failed steps.'
        ),
    ) -> None:
    ...
    @typer_pipeline.command(
        help='Start a scheduled pipeline manually via cron'
    )
    def start(
...
def _get_pipeline_logs(
    pipeline: Optional[str], limit: int, status: Optional[str] = None
):
...
def _display_logs(logs):
    ...
    for log in logs:
        ...
        table.add_row(
            pipeline_name, step, status, timestamp, output or '-', error or '-'
        )
    ...
    try:
        while True:
            ...
            for log in logs[::-1]:
                ...
                typer.echo(
                    f'📡 [{timestamp}] {pipeline_name} | {step} | {status} | {output or "-"} | {error or "-"}'
                )
...
def _clear_pipeline_logs(pipeline: Optional[str] = None) -> None:
    ...
    if pipeline:
        cursor.execute(
            'DELETE FROM pipeline_runs WHERE pipeline_name = ?', (pipeline,)
        )
    else:
